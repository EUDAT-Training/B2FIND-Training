...
### 1. Python and the Pyton scripts
* The script ```searchB2FIND.py``` the Python client ```ckanclient 0.10``` for the CKAN API. Both come already with this training package.
* Python 2.7
...
You can call all these subrequests in one call by joining them all together by `AND` :
