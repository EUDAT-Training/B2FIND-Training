...
response['count']
```
in python and we can combine this parameter easily with the two search parameters above, since they use different keys.

## Combined facetted search
We have seen how we can search across B2FIND with simple searches. But what do we have to do when we want to combine parameters for different facets?
...
### Solution 3 (by Heiner)
You are right, that multiple facets can not searched for by assigning a list of "fq":... in the data dictionary, because python will take only the last one in account. What you can do instaed is to have only one "fq" entry and combine all the facet:value pairs by AND in its value :
```sh
>>> response = ckan.call_action('package_search',{'q':'coast', "fq":"author:Dustin?Whalen and groups:pdc"})
>>> response['count']
17  
```
