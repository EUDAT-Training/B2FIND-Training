...
We present three ways how you can submit the requests :

1. *Browser* : Enter the http-requests directly in the adress field of your internet browser
2. *cURL* : Submit the requests from command line using *curl*
3. *Python ckanapi* : Submit the requests from a python shell using the python module *ckanapi*  

For detailed instruction see below.

##Prerequisites
Depending on which of the three methods you want to use, you need the tools described under points 1. to 3. Furthermore a CKAN repository which you can access msut be available, see 4. .
...
The command curl comes by default with all common linux distributions and is used 'to transfer data from or to a server', using e.g. HTTP.
...
But note that some of the searches make only sense for the B2FIND portal, e.g. the facet 'Discipline' is only defined within the B2FINE schema and comes not with the default CKAN fields.

Please replace in the following `CKAN-URL` with the URL pointing to the used CKAN endpoint.

## Usage

### 1. Submiting API Requests

The HTTP request `http-request` using directly *CKAN’s Action API*, version 3, has the following form 
...
- `function` is one of the supported CKAN API functions. While some of them, as the list commands *group_list* etc., need no further parameters, the search commnds as *package_search* expect further parameters, namely a `value` for the attribute *q* and a `facet` **and** a `value` for the attribute *fq*.  
...
2. submit the http requests from command line, using the command [cURL] (http://man.cx/curl) by excecuting ```curl <http-request>```. To transfer lengthy data as a json dictionary use the option ```-d``` and to display the response in a nicer formated JSON dictionary use piping the command in ```| python -m json.tool``` , as shown for the *Combined Search* at the end.
3. use the python package *ckanapi* : First we have to call a python shell (by ```python``` or ```ipython```) and then we need to connect to the CKAN end-point, e.g. to the B2FIND catalogue as shown here.
```sh
$ python
...
> Note : Combine several facet-value pairs by **AND** in one *fq* attribute. 
> To distribute them on serial `fq` items like
```sh
> ckan.call_action('package_search',[{'q':'<value'>},][{'fq':'<facet1>:<value1>'[ , 'fq':'<facet2>:<value2> ] ...]) 
```
would not work properly, because only the last *fq* request will be excecuted.
...
We illustrate this with the following 'search use cases', whereby we only show for the simple requests *group_list* and for the *combined search* each of the three methods in detail. For all other facets to search for we only provide the associated `action` and let it as excercise to you to apply the different query methods.
...
To list all *CKAN groups* use the action *groups_list*. E.g. to get all *communities* (realised as CKAN groups) integrated in B2FIND do the following :

1. Enter in the browser address field ```sh
http://b2find.eudat.eu/api/3/action/groups_list``` and the according request responds a json dictionary, with the CKAN groups listed in the field *"results"*.

<img align="centre" src="img/B2FIND_CKANAPI_browser_group_list.png" width="800px">

2. The same done via using *curl* from the command line
```sh
...
```
3. Using the python methode the list groups are returned as lsit of unicode values:
```sh
$ python
>>> from ckanapi import RemoteCKAN
>>> ckan = RemoteCKAN("http://b2find.eudat.eu")
...
**Excercise 1** : List all CKAN groups available in other instances by using any methode.

**Excercise 2** : List all `tags` available in ```b2find.eudat.eu``` and other CKAN instances by using any of teh three methods. (for B2FIND this results in a very long list, store the list appropriately). How many tags are there?

**Excercise 3** : List all datasets (packages) in B2FIND. This command might take some time - depending as well on the method used to submit the request. How many could you find?
...
#### Free text search
For free text search we use the attribute *q* to assign the value should searched for.

Our scientist starts to search for datasets whose metadata body contains *coast*, the associated `action` is ```package_search?q=coast``` ( or take for the two arguments of `ckan.call_action` *package_search* and *{'q':'coast'}* ).

Setting  `CKAN_URL` to *b2find.eudat.eu* results in more than 550 found datasets. The lengthy output is not shown here.

**Excercise** Try the same for other CKAN sites and by using *curl* and the *python* shell.
...
** Excercise** : Search in ```demo.ckan.org``` for all datesets belonging to the group with *id* `governo`.
...
### Combined search
We now want to combine all the single search criteria to satisfy the use case of our researcher.

I.e. we want to filter out all datasets that fulfill each of the following criteria within **one call** (we only list here the facets which working properly by directly sending combined HTTP CKAN API requests):
...
**Excercise** : Play around with all three 'methods' and discuss in groups : 
What are the pro's and con's, especially for such a lengthy and complex request ? 
**Excercise** : Set up your own use cases and try to transpose them in CKAN requests. Docuemnt and discuss your experiences by submitting your requests.
