...
This document describes how you can interact with a CKAN site by directly submitting HTTP requests using the CKAN API version 3, which the B2FIND API and the search via the GUI (see [Search GUI](./05.b-search-GUI.md)) and the command line interface (see [Search CLI](./05.c-search-CLI.md)) are based. For details see the [API guide](http://docs.ckan.org/en/latest/api/index.html) of CKAN.
...
>>> response = ckan.call_action('package_search',{'q':'coast',
	"fq":"groups:pdc AND Discipline:Environmental?Science AND author:Dustin?Whalen", 
