...
In this sub module we show how requests can be submitted using three different methods:
...
Note that some of the searches only make sense for the B2FIND portal, e.g. the facet 'Discipline' is only defined within the metadata schema of the B2FIND service. Please replace in the following the `CKAN-URL` variable with the URL of the used CKAN installation.
...
1. In a browser, enter the following address: `http://b2find.eudat.eu/api/3/action/group_list`. The corresponding request responds with a JSON dictionary, with the CKAN groups listed in the field `results`.
...
**Exercise 2**: List all `tags` available in ```b2find.eudat.eu``` and other CKAN instances by using the faunction `tag_list` and any of the three methods (for B2FIND this results in a very long list, store the list appropriately). How many tags are there ?
