...
- *CKAN_URL* is the URL of the CKAN site in which should be searched. In the examples below we use the B2FIND portal ```b2find.eudat.eu``` and if it make sense the demo site ```demo.ckan.org```. But you can try the requests out also with any other CKAN site.  
- *function* is one of the supported CKAN API functions. While some of them, as the list commands `group_list` etc., need no further parameters, the search commnds as `package_search` expect further parameters as the *facet* and *value* to be searched for.  
...
In the sub following sections we explain first how for single facets can be searched for and finally we provide the result of a full combined search, which leads hopefully to the desired results our researcher needs for her own study. 
...
##### `Community` (or `Group`)
In B2FIND *Communities* correspond to CKAN *groups*. 

E.g. the request to filter out all datasets  belonging to the community `PDC` in the B2FIND catalogue is

```sh
http://b2find.eudat.eu/api/3/action/package_search?fq=groups:pdc
```

> Excersice 1 : Search in ```demo.ckan.org``` for all datesets belonging to the group with *id* `governo`.
> Exercise 2 : Try to get out the *display_name* of the group with *id* governo. (Hint : use the function *group_show* )  

##### Discipline
**NOTE**, the following searches are dependent on the existence of the facet *Discipline*. While it is defined in the B2FIND schame it might likely not exist in your CKAN instance. Therefore use only *b2find.eudat.eu* instead.
