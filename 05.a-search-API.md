...
2. submit the http requests from command line, using the command [cURL] (http://man.cx/curl):
```sh
curl <http-request>
```
To transfer lengthy as a json dictionary use the option ```-d``` and to display the response in a nicer formated JSON dictionary use piping the command in ```| python -m json.tool``` , as shown for the *Combined Search* at the end.
