...
1. Enter the http-requests directly in the adress field of your internet browser
2. Submit the requests from command line using *curl*
3. Submit the requests from a python shell using the python module *ckanapi*  

##Prerequisites
Depending on which of the three methods you want to use, you need
...
3. use the python package *ckanapi* : First we have to call a python shell (by ```sh python``` or ```sh ipython```) and then we need to connect to the CKAN end-point, e.g. to the B2FIND catalogue as shown here.
```sh
$ python
...
The data for the facetted search are specied in a dictionary, where the element 'q' contains the value for teh free text search and 'fq' the facet-value pairs to be searched for, i.e.
...
**Excercise 2 ** :
- List all `tags` available in ```b2find.eudat.eu``` and other CKAN instances by using any methode. (for B2FIND this results in a very long list, store the list appropriately). How many tags are there?

**Excercise 3** : List datasets (packages)
- List all datasets (packages) in B2FIND, this command might take some time. How many could you find?
...
As <function> we use here *package_search*. 
...
#### Free text search
For free text search we use the attribute 'q' to assign the value shoudl searched for.

If you want to search for datasets whose metadata body contains *coast*, the associated action is ```sh package_search?q=coast``` or as arguments of ckan.call_action *package_search* and *{'q':'coast'}*.
...
#### **Textual** facets
For facetted search we use the attribute 'fq' to assign the value should be searched for.
